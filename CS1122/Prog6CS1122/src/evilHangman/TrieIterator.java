...
public class TrieIterator implements Iterator<String>{
	...
	public String next(){
		...
		// sanity check, can't call next if there are no more words in the Trie
	...
	public void remove(){
		...
		// sanity check, can't remove a word that hasn't been visited!
		if (visited == 0) return;
		...
		while (!curr.hasChildren() && !curr.isEndOfWord() && !curr.isRoot()){
		...
		// point to the new iterator's pointers
		curr = iter.curr;
		stack = iter.stack;
