...
public class HangmanPanel extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		int msgY = 20;
		if (game.gameWon()) {
			g.drawString("Player " + game.currentPlayer() + " Wins!!!", msgX,
					msgY);
		}
		if (game.gameLost()) {
		...
		if (game.gameActive()) {
			g.drawString("Player " + game.currentPlayer() + "'s Turn", msgX,
					msgY);
		} else {
		...
		// draw current word
		String currWord = fixString(game.getExposedLetters());
		Font serifFont = new Font("Times", Font.PLAIN, 30);
		g.setFont(serifFont);
		g.setColor(Color.black);
		g.drawString(currWord, 200, groundLine - 10);
	...
	String fixString(String str) {
		...
		for (int i = 0; i < str.length(); ++i) {
			if (str.charAt(i) != ' ')
				letters[str.length() - i - 1] = str.charAt(i);
		}
