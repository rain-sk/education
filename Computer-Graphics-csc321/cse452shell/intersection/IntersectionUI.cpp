...
void IntersectionUI::changeShape( ShapesUI::ShapeType type )
{
    ...
    case ShapesUI::SHAPE_SPHERE : 
		current = new Sphere(100);
		break;
    case ShapesUI::SHAPE_CYLINDER : 
		current = new Cylinder(20,1);
		break;
    case ShapesUI::SHAPE_CONE : 
		current = new Cone(20,1);
		break;
...
void IntersectionUI::writeTest() const {
    ...
    for (int i = seed; i < (seed + 1638); i++) {
		...
		Shape *temp = new Cube(1);
        cubeHr = temp->intersect(p, dir);

		temp = new Cylinder(20,1);
        cylinderHr = temp->intersect(p, dir);

		temp = new Cone(20,1);
        coneHr = temp->intersect(p, dir);

		temp = new Sphere(20);
        sphereHr = temp->intersect(p, dir);
